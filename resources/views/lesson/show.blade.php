...
<div class="container mt-5 mb-5">
        ...
        <tbody>
            @foreach($lesson->tests as $test)
                <tr>
                    ...
                    <td>{{ $test->title }}</td>
                    <td>
                        <a href="{{ route('test.show', $test->id) }}" class="btn btn-primary mx-1">Show</a>
                        <a href="{{ route('test.edit', $test->id) }}" class="btn btn-warning mx-1">Edit</a>
                        <form action="{{ route('test.destroy', $test->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <a href="{{ route('test.create', ['lesson_id' => $lesson->id]) }}" class="btn btn-success">Add Test</a>
</div>
</div>
</div>
@endsection
